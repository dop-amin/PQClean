...
int PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx(
        ...
        size_t ctxlen,
        const uint8_t *sk) {
    return PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx_trace(
...
*              - uint8_t *sig:   pointer to output signature (of length PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES)
...
rej:
    ...
    if (trace) {
        z_cs1 = z;
        PQCLEAN_MLDSA65_CLEAN_polyvecl_invntt_tomont(&z_cs1);
